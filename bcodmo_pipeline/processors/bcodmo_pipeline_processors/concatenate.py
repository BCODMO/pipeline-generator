...
def concatenator(resources, all_target_fields, field_mapping, include_resource_name, new_field_name):
    for resource_ in resources:
        ...
        for row in resource_:
            ...
            if include_resource_name:
                processed[new_field_name] = res_name
            yield processed
...
def concatenate(
    ...
    def func(package):
        ...
        if include_resource_name and new_field_name in needed_fields:
            raise Exception(f'new_field_name "{new_field_name}" field name already exists')
...
def flow(parameters):
    return Flow(
        concatenate(
            ...
            parameters.get('include_resource_name', False),
            parameters.get('new_field_name', 'res_name'),
        ),
        update_resource(
